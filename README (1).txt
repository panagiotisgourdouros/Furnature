General info:
1) Backend was built with Java 8, you might wanna select appropriate JDK
	under groupproject properties-> compile-> Java Platform: JDK 1.8
...
3) We have 3 roles admins, customers, superuser
	-> superuser can perform crud operations on admins
		...
		username:peoplecert & password:P1234567

-------------------------------------------------------------------------------------------------------------------------------
Database Setup:
  General Steps: MySQL server should run under localhost:3306,
		 otherwise new connection from backend should be made to the database
  ...
  Detailed Steps: Visual Studio Code IDE steps:
		  1)In order to run the project you need a live server extension.
		   ...
		   2)Open project's files:		
			...
			2.b Navigate to Furnature project, select frontendFurnature folder and click Select Folder
